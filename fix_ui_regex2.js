...
// 1. Add chat toggle button
const regex1 = /(\$\{isVisited \? "\+ ADD AGAIN" : "\+ ADD VISIT"\}\s*<\/button>)/;
if (ui.match(regex1)) {
  ui = ui.replace(regex1, `$1
  ...
  console.log('Regex1 applied');
}

// 2. Add chat container
const regex2 = /(<\/div>\s*<\/div>\s*)`;/m;
if (ui.match(regex2)) {
  ui = ui.replace(regex2, `$1
